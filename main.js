...
Parse.Cloud.beforeSave("Group", function(request, response) {
	...
	var keywords_desc = group.get("description").split(/b/);	//just in case we want to search depending on the group's description
    keywords_desc = _.map(keywords_desc, toLowerCase);
    keywords_desc = _.filter(keywords_desc, function(w) { return w.match(/^w+$/) && ! _.contains(stopWords, w); });
 
    var hashtags = group.get("name").match(/#.+?b/g);
    hashtags = _.map(hashtags, toLowerCase);
	
	var hashtags_desc = group.get("description").match(/#.+?b/g);
    hashtags_desc = _.map(hashtags_desc, toLowerCase);
 
    group.set("keywords", keywords);
    group.set("hashtags", hashtags);
	group.set("keywords_desc", keywords_desc);
	group.set("hashtags_desc", hashtags_desc);
    response.success();
});